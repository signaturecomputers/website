...
export default function CartPage() {
    const { cart, savedItems, removeFromCart, updateQuantity, toggleWindowsInstallation, removeCarePack, cartTotal, cartCount, removeFromSaved, moveToCart, saveForLater, isLoading } = useCart();
                                ...
                                <div key={item.id} className="flex gap-6 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-800">
                                    ...
                                    <div className="flex-1 flex flex-col justify-between">
                                        <div>
                                            ...
                                            {/* HP Care Pack Add-on */}
                                            {item.carePack && (
                                                <div className="mt-2 pl-4 border-l-2 border-emerald-500 bg-emerald-50/50 dark:bg-emerald-950/20 rounded-r-lg py-2 pr-3">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-2">
                                                            <svg className="w-4 h-4 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                                            </svg>
                                                            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{item.carePack.title} ({item.carePack.duration})</span>
                                                        </div>
                                                        <div className="flex items-center gap-3">
                                                            <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">+₹{(item.carePack.price * item.quantity).toLocaleString('en-IN')}</span>
                                                            <button
                                                                onClick={() => {
                                                                    removeCarePack(item.id);
                                                                    toast.info('HP Care Pack removed');
                                                                }}
                                                                className="text-red-500 hover:text-red-700 transition-colors p-1"
                                                                title="Remove HP Care Pack"
                                                            >
                                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                                                                </svg>
